...
describe("selectHydrationIds", () => {
  it("always includes rated titles, even beyond the limit", () => {
    const candidates = [
      { id: 1, lastActivityAt: new Date("2020-01-01"), isRated: true },
      { id: 2, lastActivityAt: new Date("2020-01-01"), isRated: true },
    ];
    const selected = selectHydrationIds({ candidates, mustIncludeIds: [], limit: 1 });
  ...
  it("always includes explicitly must-include titles (e.g. the most rewatched movie)", () => {
    const candidates = [
      { id: 1, lastActivityAt: new Date("2010-01-01"), isRated: false },
      { id: 2, lastActivityAt: new Date("2024-01-01"), isRated: false },
    ];
    const selected = selectHydrationIds({ candidates, mustIncludeIds: [1], limit: 1 });
    expect(selected).toContain(1);
  });

  it("fills remaining slots with the most recently active unrated titles", () => {
    const candidates = [
      { id: 1, lastActivityAt: new Date("2020-01-01"), isRated: false },
      { id: 2, lastActivityAt: new Date("2024-01-01"), isRated: false },
      { id: 3, lastActivityAt: new Date("2022-01-01"), isRated: false },
    ];
